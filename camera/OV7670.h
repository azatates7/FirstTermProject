...
const struct regval_list ov7670_default_regs[] PROGMEM = { // from the linux driver
  ...
  */
  ...
  { 0x79, 0x01 }, { 0xc8, 0xf0 }, /* Extra-weird stuff.  Some sort of multiplexor register */
  { 0x79, 0x0f }, { 0xc8, 0x00 },
  { 0x79, 0x10 }, { 0xc8, 0x7e },
  { 0x79, 0x0a }, { 0xc8, 0x80 },
  { 0x79, 0x0b }, { 0xc8, 0x01 },
...
void writeReg(uint8_t reg, uint8_t dat){  // send start condition
  twiStart();
...
void setColor(void){
  wrSensorRegs8_8(yuv422_ov7670); // wrSensorRegs8_8(qvga_ov7670);
}
...
void arduinoUnoInut(void) {
  ...
  * This will be on pin 11*/
  ...
  _delay_ms(3000); // set up twi for 100khz
...
static void captureImg(uint16_t wg, uint16_t hg){
  ...
  while ((PIND & 8));//wait for low
    y = hg;
  while (y--){
        x = wg; // while (!(PIND & 256)); // wait for high
    while (x--){
